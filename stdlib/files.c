...
static void file_destroy(presource *resource) {
    ...
    if (file->file != NULL) (void)fclose(file->file);
    pphp_free(file);
}
...
int pphp_file_read_all(const char *path, char **bytes, size_t *length) {
    ...
    *length = 0U;
    ...
    if (fseek(file, 0L, SEEK_END) != 0 ||
        ...
        (unsigned long)measured > (unsigned long)PPHP_STR_MAX ||
    ...
    read_count = fread(buffer, 1U, (size_t)measured, file);
    if (read_count != (size_t)measured || ferror(file)) {
        pphp_free(buffer);
        (void)fclose(file);
        return 0;
    }
...
static int call_whole_file(pphp_state *state, const pstring *name,
    ...
    if (name_is(name, "file_put_contents")) {
        ...
        written = fwrite(data->data, 1U, data->length, file);
...
static int call_path_operation(pphp_state *state, const pstring *name,
    ...
    if (name_is(name, "file_exists")) {
        ...
        *result = pv_bool(stat(first, &info) == 0);
        return 1;
    }
    if (name_is(name, "unlink") || name_is(name, "rmdir")) {
        int status;
        if (count != 1U || first == NULL) return invalid_arguments(state, name);
        status = name_is(name, "unlink") ? unlink(first) : rmdir(first);
        *result = pv_bool(status == 0);
        return 1;
    }
    if (name_is(name, "mkdir")) {
        ...
        *result = pv_bool(mkdir(first, (mode_t)(mode & 0777)) == 0);
        return 1;
    }
    if (name_is(name, "rename")) {
        ...
        *result = pv_bool(rename(first, second) == 0);
        return 1;
    }
    if (name_is(name, "filesize")) {
        struct stat info;
        if (count != 1U || first == NULL) return invalid_arguments(state, name);
        if (stat(first, &info) != 0 || info.st_size < 0) {
            *result = pv_bool(0);
        } else {
#if PPHP_INT64
            *result = pv_int((pphp_int)info.st_size);
#else
            *result = (uint64_t)info.st_size > (uint64_t)INT32_MAX
                          ? pv_float((pphp_float)info.st_size)
                          : pv_int((pphp_int)info.st_size);
#endif
        }
        return 1;
    }
...
static int call_stream(pphp_state *state, const pstring *name,
    ...
    if (count == 0U) return invalid_arguments(state, name);
    {
        ...
        if (name_is(name, "fread")) {
            ...
            got = fread(buffer, 1U, (size_t)requested, resource->file);
            string = ps_new(buffer, got);
            pphp_free(buffer);
            if (string == NULL) {
        ...
        if (name_is(name, "fwrite")) {
            ...
            data = pv_to_string(arguments[1]);
            if (data == NULL) return invalid_arguments(state, name);
            written = fwrite(data->data, 1U, data->length, resource->file);
            ps_destroy(data);
            *result = pv_int((pphp_int)written);
            return 1;
        }
        if (name_is(name, "fgets")) {
            ...
            if (fgets(buffer, (int)limit + 1, resource->file) == NULL) {
                pphp_free(buffer);
                *result = pv_bool(0);
                return 1;
            }
        ...
        if (name_is(name, "feof")) {
            if (count != 1U) return invalid_arguments(state, name);
            *result = pv_bool(feof(resource->file));
            return 1;
        }
        if (name_is(name, "fseek")) {
            ...
            *result = pv_int((pphp_int)fseek(resource->file,
                                             (long)arguments[1].as.i,
                                             whence));
            return 1;
        }
        if (name_is(name, "ftell")) {
            long position;
            if (count != 1U) return invalid_arguments(state, name);
            position = ftell(resource->file);
            *result = position < 0L ? pv_bool(0)
...
static int call_scandir(pphp_state *state, const pstring *name,
    ...
    while ((entry = readdir(directory)) != NULL) {
        pstring *item = ps_new(entry->d_name, strlen(entry->d_name));
        pvalue value;
...
scan_oom:
    ...
    (void)count;
    (void)result;
    pphp_runtime_error(state, 0U, "%.*s() requires an enabled filesystem",
                       (int)name->length, name->data);
    return -1;
#endif
}
